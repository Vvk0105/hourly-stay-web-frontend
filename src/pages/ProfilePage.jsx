...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        const n = res.data.name || `${res.data.first_name || ''} ${res.data.last_name || ''}`.trim()
        setName(n)
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setProfileData(res.data)
      dispatch(updateUser({ name: `${res.data.first_name || ''} ${res.data.last_name || ''}`.trim() }))
  ...
  const displayName = name || data?.name || data?.email?.split('@')[0] || 'User'
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold text-gray-800 text-sm truncate">{data?.email || '—'}</p>
              </div>
            ...
            <div className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 hover:border-brand-200 transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold text-gray-800 text-sm">{data?.phone || '—'}</p>
              </div>
              <button
            ...
            <div className="space-y-3 text-sm">
              ...
              {data?.id && (
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-xs text-gray-500 truncate max-w-[200px]">{data.id}</span>
                </div>
              )}
